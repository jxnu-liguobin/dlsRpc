...
import io.github.dlsrpc.common.utils.{CheckCondition, SuperClassUtils}
import io.github.dlsrpc.core.api.{SendMessage, Serializer}
...
class ServerMessageHandlerImpl @Inject()(serializer: Serializer, channel: ServerChannel) extends ServerMessageHandler with LazyLogging {
  ...
  override def processor(request: Array[Byte], receiveMessage: SendMessage): Unit = {
    ...
    CheckCondition.conditionException(this.serviceBeans == null, "bean can't be null")
    //接口消息并反序列化解码拿到真正的请求
    ...
    CheckCondition.conditionException(processorBean == null, "bean can't be found")
