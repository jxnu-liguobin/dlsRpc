package io.growing.dls.discovery.consul
...
import io.growing.dls.commons.ConsulBuilder
import io.growing.dls.discovery.ServiceDiscovery
import io.growing.dls.loadbalancer.RandomLoadBalancer
...
class ConsulServiceDiscovery(consulAddress: ServiceAddress) extends ServiceDiscovery with LazyLogging {
  ...
  private[this] def buildLoadBalancer(healthServices: JList[HealthService]): RandomLoadBalancer[ServiceAddress] = {
